...
| `volto/components/manage/Blocks/Image/View.jsx` | EEA image rendering, preview-image links, copyright overlays, and custom link handling. | Moderate upstream delta. | `needs rebase` | Keep the current dual-support fix (`UniversalLink item={hrefItem}`) so `linkWithHash` survives in rendered image links, then rebase onto Volto 18 view component and keep only rendering differences still needed. Cypress test `08-object-browser-widget.cy.js` now covers this path. | Paired image schema/edit shadows |
