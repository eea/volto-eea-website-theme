...
| `@plone/volto-slate/blocks/Table/TableBlockView.jsx` | Ensures public table rendering matches EEA slate styling. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep. | Inline TODO only |
| `@plone/volto-slate/blocks/Text/TextBlockView.jsx` | Keeps EEA-specific text block rendering and text style variants. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep. | None |
| `@plone/volto-slate/editor/SlateEditor.jsx` | Safe value handling: `getEditorValue()` normaliser, `resetNodes()` for prop-change sync, `cloneDeep` to prevent mutation. Absolute imports required for customisation path resolution. `spellCheck=true`, safe `onClick` guard. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — all additions are self-contained. `resetNodes` and `getEditorValue` absent from V18 upstream; EEA shadow provides them for both versions. Absolute imports (`@plone/volto-slate/…`) resolve correctly via the Jest + Webpack alias on both V17 and V18. No unit tests (Slate requires heavy mocking). | README note, changelog refs `#264239` |
| `@plone/volto-slate/editor/plugins/Table/less/public.less` | EEA table public styling override. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep. | None |
...
| `@plone/volto-slate/elementEditor/utils.js` | `setTimeout` wrapper for selection restore in `_unwrapElement`; `getSavedSelection` fallback; `_getActiveElement` cursor-at-start guard. | V17 = V18 upstream (0 diff); the installed V18 does NOT have the `setTimeout` fix either. | `compatible` ✅ verified | Keep for both V17 and V18 — the EEA shadow improves selection-restore stability on both versions. Remove after the upstream absorbs these fixes. | README, changelog |
| `@plone/volto-slate/utils/blocks.js` | Better non-block-node wrapping in `convertBlockToSlate` (manual reduce instead of `Transforms.wrapNodes`); replaces `clearList` with `clearFormatting`; absolute import for `makeEditor`. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — EEA logic is entirely self-contained. Absolute import resolves correctly. No unit tests (Slate transform testing requires editor context). | Changelog refs `#273976`, `#265782`, `#261770` |
| `@plone/volto-slate/utils/selection.js` | Shadow patch for #8347: fixes `isCursorAtBlockStart` to only return true at the first leaf of the block, not at offset 0 of any leaf. | V17 = V18 upstream (0 diff in selection.js). Installed V18 (18.10.0) has the bug. | `compatible` ✅ verified | **Remove** once a volto-slate release containing the fix ships and the dependency is bumped in `frontend/package.json`. | Upstream PR #8355 |
| `@plone/volto-slate/utils/volto-blocks.js` | Shadow patch for #8347: fixes cursor position in `mergeSlateWithBlockBackward` when merging same-type blocks. Slate normalization merges adjacent text nodes, making the original cursor path `[idx, lastNode.children.length]` invalid. Fix points cursor into the surviving text node at offset = its text length. | V17 = V18 upstream (0 diff). Installed V18 (18.10.0) has the bug. | `compatible` ✅ verified | **Remove** once a volto-slate release containing the fix ships and the dependency is bumped in `frontend/package.json`. | Upstream PR #8355 (pending) |
| `@plone/volto-slate/blocks/Text/keyboard/joinBlocks.js` | Shadow patch for #8347: guards `joinWithPreviousBlock` against non-slate previous blocks (e.g., image). Without this, `getBlockEndAsRange` and `mergeSlateWithBlockBackward` crash because non-slate blocks have no `value` property. | V17 = V18 upstream (0 diff). Installed V18 (18.10.0) has the bug. | `compatible` ✅ verified | **Remove** once a volto-slate release containing the fix ships and the dependency is bumped in `frontend/package.json`. | Upstream PR #8355 (pending) |
| `@root/theme.js` | Replaces `semantic-ui-less/semantic.less` with `@eeacms/volto-eea-design-system/semantic.less`; uses absolute `@plone/volto/` prefix for Pastanaga extras. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — both imports resolve under both V17 and V18. Build-time LESS verification needed when switching to V18 Webpack config. | None |
| `@root/toast.less` | EEA Design System toast styling (positioning, colors, responsive width). No upstream counterpart — `@root` customization. | N/A (no upstream file). | `compatible` ✅ verified | Keep — pairs with `Toast.jsx` shadow. EEA-specific LESS only. | Refs `#294806` |

### Manage UI: blocks (additional)

| Shadow | Why customized in 17 | Volto 18 delta | Status | Next action | Support |
| --- | --- | --- | --- | --- | --- |
| `volto/components/manage/Blocks/Block/Order/Item.jsx` | Uses `isBoolean` from lodash instead of `typeof === 'boolean'` for required-block check. Comment refs Volto #6481 (PRs #8124, #8125). | V17 = V18 = V19 upstream (all use `typeof`). | `compatible` ✅ verified | Keep — functionally identical to upstream (`isBoolean` ≡ `typeof === 'boolean'`). Remove when PRs #8124/#8125 merge and release. | Refs `#302095`, Volto #6481 |

| Shadow | Why customized in 17 | Volto 18 delta | Status | Next action | Support |
| --- | --- | --- | --- | --- | --- |
| `volto/actions/vocabularies/vocabularies.js` | Backports a newer vocabulary-fetch implementation from Volto 19.x. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — 4 unit tests pass. Compare directly with Volto 19 before removing. | `vocabularies.js.md`, `.diff`, test shadow |
...
### Manage UI: contents, sharing, toast, and clipboard

| Shadow | Why customized in 17 | Volto 18 delta | Status | Next action | Support |
| --- | --- | --- | --- | --- | --- |
| `volto/components/manage/Contents/ContentsItem.jsx` | Precomputes `isExpired`/`isScheduled` once per render; flexbox layout for title + badge collision fix. | Small upstream delta. | `compatible` ✅ verified | Keep — EEA layout fix is self-contained. Has `.diff` and `.md`. | `.diff`, `.md`, refs `#303325` |
| `volto/components/manage/Form/BlocksToolbar.jsx` | Fixes clipboard rehydration on Volto 18: `redux-localstorage-simple` key format changed from monolithic `blocksClipboard` to split `blocksClipboard.cut`/`.copy`. Adds eager `loadFromStorage()` on mount. | V18 changed localStorage key format. | `compatible` ✅ verified | Keep — has detailed `.md` explaining the Volto 18 localStorage key migration. Remove when upstream fixes the rehydration. | `.diff`, `.md`, PR #377 |
| `volto/components/manage/Form/blocksClipboardUtils.js` | Local support file for `BlocksToolbar.jsx`. Provides `cloneBlocks` and `loadBlocksClipboardFromStorage` that handle both V17 (monolithic key) and V18 (split key) localStorage formats. | No upstream counterpart (local module). | `support file` | Keep with `BlocksToolbar.jsx` shadow. Not a shadow itself. | Paired with `BlocksToolbar.jsx` |
| `volto/components/manage/Sharing/Sharing.jsx` | Null-safe access to `state.content.data.title` via optional chaining (`?.title || ''`). Fixes Sentry 440198 (1,253 events). | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — one-line null-safety fix. Has `.diff` and `.md`. | `.diff`, `.md`, Sentry 440198 |
| `volto/components/manage/Sidebar/objectBrowserSelection.js` | Local support file for `ObjectBrowserBody.jsx`. Provides `isItemAlreadySelected` and `isSelectableObjectBrowserItem` helpers for selection limit enforcement. | No upstream counterpart (local module). | `support file` | Keep with `ObjectBrowserBody.jsx` shadow. Not a shadow itself. | Refs `#301881`, Cypress `10-object-browser-selection-limit.cy.js` |
| `volto/components/manage/Toast/Toast.jsx` | Replaces Volto's toast with Semantic UI `Message` component for EEA Design System alignment. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — EEA Design System toast rendering. Pairs with `@root/toast.less`. | Refs `#294806` |

| Shadow | Why customized in 17 | Volto 18 delta | Status | Next action | Support |
| --- | --- | --- | --- | --- | --- |
| `volto/components/manage/Controlpanels/Groups/RenderGroups.jsx` | Shows `title \|\| groupname` in the groups table instead of only `groupname`. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — single-line additive change. Test converted from stale snapshot (renderer.create) to `@testing-library/react` behavior assertions; snapshot file deleted. 5 unit tests pass. | `.diff`, test |
| `volto/components/manage/Diff/DiffField.jsx` | Uses `useStore()` to get current Redux state instead of `window.__data`; resolves widgets by behavior/factory. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — uses `useStore()` (standard react-redux) and `useSelector()`, both available on V17 and V18. No unit test exists; complex rendering tested by `DiffField.diff` and `DiffField.txt` support artifacts. | `.diff`, `.txt` |
| `volto/components/manage/Display/Display.jsx` | Restricts who can change layout based on `config.settings.eea.rolesWhoCanChangeLayout`. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — uses `jwtDecode`, `getUser` action, and standard connect() pattern; all available on V17 and V18. No unit test required (role-gate logic is integration-level). | `Readme.md` |
| `volto/components/manage/History/History.jsx` | Adds copied-to/copied-from banners, `getPathname()` URL guard, direct (tree-shaken) imports, `createPortal` instead of `react-portal`. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — already uses `createPortal` from `react-dom` (the V18-correct approach). All direct imports resolve in both versions. No unit test required beyond integration-level behavior. | `.diff`, changelog refs `#289335`, `#297683` |
| `volto/components/manage/Sidebar/ObjectBrowserBody.jsx` | Sets default alt to `item.title \|\| item.description` and `copyright: item.rights`; simplifies `maximumSelectionSize` guard (`<` not `<=`). | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — purely additive EEA behaviour. | Sidebar `README.md`, inline comment |
| `volto/components/manage/Sidebar/ObjectBrowserNav.jsx` | Shows inline mini image preview for `@type === 'Image'` items in the file picker nav. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — additive UI addition. | Sidebar `README.md` |
...
| `volto/components/manage/Toolbar/More.jsx` | Removes "Links and references" menu item; hides certain items for `ims_indicator` content type; adds `key` prop to Pluggable fragments. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — all changes are additive/subtractive guards, no new API used. No unit test required (menu integration). | None |
| `volto/components/manage/UniversalLink/UniversalLink.jsx` | Removes `noreferrer` for SEO; adds `item.linkWithHash` support (paired with `ObjectBrowserWidget`); null-safe download URL check; fixes empty-`@id` guard. | V17 = V18 upstream (0 diff); file is still `.jsx`, not `.tsx` — audit note about `.tsx` was incorrect. | `compatible` ✅ verified | Keep — all changes are self-contained. `linkWithHash` patch is paired with `ObjectBrowserWidget` (already rebased). | Inline comment in shadow |
...
| `volto/components/manage/Workflow/Workflow.jsx` | Adds `editingProgressSteps` guard on transitions, `workflowLoaded` for version redirect, filters `markForDeletion`, error toast. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — uses `useSelector`, `useHistory` (react-router-dom v5, same on both), `injectLazyLibs` (same API). Unit tests updated: barrel mock + `editingProgress`/`workflow.get` store slices; 4 tests pass. | `README.txt`, test |

### Theme UI

| Shadow | Why customized in 17 | Volto 18 delta | Status | Next action | Support |
| --- | --- | --- | --- | --- | --- |
| `volto/components/theme/Breadcrumbs/Breadcrumbs.jsx` | EEA breadcrumb markup, `contentTypesAsBreadcrumbSection` support using `portal_type` from the reducer shadow. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — barrel imports and `connect()` pattern unchanged in V18. Test + snapshot pass. | Test, snapshot |
| `volto/components/theme/Comments/Comments.jsx` | EEA comments — class component fork with threading, avatars, reply/edit/delete, `react-portal` for toolbar. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — `react-portal` installed in both V17/V18; all imports stable. Tests pass (4 cases). | Test, snapshot, `comments.less` |
...
| `volto/components/theme/ContactForm/ContactForm.jsx` | Redirects to `config.settings.contactForm`; EEA version does not use `react-portal` or `useHistory` unlike upstream. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — simpler than upstream; `UniversalLink` barrel import stable in V18. Test + snapshot pass. | Test, snapshot |
...
| `volto/components/theme/EventDetails/EventDetails.jsx` | Uses `expandToBackendURL` instead of `flattenToAppURL` for event URL construction. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — `expandToBackendURL` is in `@plone/volto/helpers` on both versions. | None |
| `volto/components/theme/Footer/Footer.jsx` | Backend-driven footer links, social/contact actions, and EEA footer behavior. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — no barrel issues, stable imports. Smoke-test data coming from backend settings during Cypress run. | Changelog refs `#151856`, `#253198` |
| `volto/components/theme/Header/Header.jsx` | Full EEA header fork with design-system Header, EEA logo, language switcher, navigation settings. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — uses `connect`, `withRouter`, `useDispatch`/`useSelector`, `UniversalLink`; all stable in V18. Test + snapshot pass. | Test, snapshot, paired `LanguageSwitcher.jsx` |
...
| `volto/components/theme/Image/Image.jsx` | Custom image scale selection and support for `preview_image_link` / `base_path`. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — uses `flattenScales`, `flattenToAppURL` from `@plone/volto/helpers`; both available in V18. | `README.md` |
...
| `volto/components/theme/AlternateHrefLangs/AlternateHrefLangs.jsx` | Patch for Volto #7309 (crash when `content.language` is undefined). | Upstream already has the fix (`content.language?.token`). Shadow only adds a comment block — code is identical. | `upstream absorbed` ✅ | **Remove** — the installed V18 (18.35.0) already contains the optional-chaining fix. The shadow adds zero runtime change. | Volto #7309, PRs #8093, #8094 |
...
| `volto/components/theme/View/DefaultView.jsx` | EEA default page view with context navigation integration and custom layout. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — all imports stable in V18. No unit test (rendering integration). | Paired `style.less` |
...
| `volto/components/theme/View/EventView.jsx` | EEA event page view with permalink and layout changes. | V17 = V18 upstream (0 diff). | `compatible` ✅ verified | Keep — standard imports. No unit test (integration). | Paired with `EventDatesInfo.jsx` and `style.less` |
...
6. ✅ Verify editorial shell components:
   `Toolbar/More.jsx`, `Display.jsx`, `History.jsx`, `Workflow.jsx`, `Diff/DiffField.jsx`, `Controlpanels/Groups/RenderGroups.jsx` — all verified compatible. V17/V18 upstream is identical for all six files. `RenderGroups.test.jsx` rewritten to `@testing-library` (snapshot deleted, 5 behavioral tests); `Workflow.test.jsx` fixed with barrel mock + EEA store slices (4 tests). Total 9 new unit tests pass.
7. ✅ Verify the public EEA theme stack:
   `Header.jsx`, `Breadcrumbs.jsx`, `DefaultView.jsx`, `EventView.jsx`, `Footer.jsx`, `Comments.jsx`, `ContactForm.jsx` — all verified compatible. V17/V18 upstream identical for all seven files. All existing tests pass (15 tests across 4 suites). No rebases required.
8. ✅ Verify `@plone/volto-slate` shadows:
   All 8 files verified — V17/V18 upstream identical throughout. `SlateEditor.jsx` (safe value sync, `resetNodes`), `utils/blocks.js` (non-block wrapping, `clearFormatting`), `elementEditor/utils.js` (selection-restore `setTimeout`), `render.jsx` (cx-merge), and the four compatible read-only shadows all work on both versions. Absolute imports correct for customisation resolution. No rebases required.
