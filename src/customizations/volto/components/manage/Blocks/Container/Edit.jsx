...
const ContainerBlockEdit = (props) => {
  ...
  const onSelectBlock = (id, multiple, event) => {
    ...
    const current = multiSelected.filter((item) => items.includes(item));
    const anchor = current[0] || selectedBlock || id;
    let next;
    if (event.shiftKey) {
      const start = Math.max(0, items.indexOf(anchor));
      const end = items.indexOf(id);
      next = items.slice(Math.min(start, end), Math.max(start, end) + 1);
    } else {
      const selection = current.length
        ? current
        : selectedBlock
          ? [selectedBlock]
          : [];
      next = selection.includes(id)
  ...
  const containerProps = {
    ...
    onSelectTemplate,
    selectedBlock,
    setSelectedBlock: (id) => onSelectBlock(id),
    templates,
  };
    ...
    <>
      ...
      {selected && (
        <BlocksToolbar
          ...
          onChangeBlocks={changeFormData}
        />
      )}
      <SidebarPortal
        selected={selected && !selectedBlock && !multiSelected.length}
      >
